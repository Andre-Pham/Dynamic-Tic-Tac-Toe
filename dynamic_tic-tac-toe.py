...
def generate_next_perm(permutation):
    '''
...
def generate_permutations(vlist):
    '''
...
def generate_score_for_simulation(simulation, required_in_a_row):
        ...
        for index1, row in enumerate(copy_board):
            for index2, element in enumerate(row):
                if element == '_':
                    ...
                    permutation.pop()
        if check_if_winner_exists(copy_board, required_in_a_row):
    ...
    for outcome in simulated_outcomes:
        if check_if_winner_exists(outcome, required_in_a_row, 'computer'):
            score += 1
        if check_if_winner_exists(outcome, required_in_a_row, 'player'):
            score -= 1
    return score

def check_if_first_turn(board):
    '''
...
def subtract_scores_indices(board):
    '''
    ...
    #These are the indices of the flattened baord that form the perimeter.
    perimeter_indices = set()
    for i in range(0, num_of_rows*num_of_cols, num_of_cols):
        perimeter_indices.add(i)
        perimeter_indices.add(i-1+num_of_cols)
    for i in range(0, num_of_rows):
        perimeter_indices.add(i)
        perimeter_indices.add(i+num_of_rows*(num_of_cols-1))
...
def find_optimal_computer_turn(board, required_in_a_row, speed_turn_one):
    '''
    ...
    for sim in list_of_options_simulated:
        if check_if_winner_exists(sim, required_in_a_row):
    ...
    if speed_turn_one and check_if_first_turn(board):
        return [(len(board) - 1)//2, (len(board[0]) - 1)//2]
    ...
    for num, sim in enumerate(list_of_options_simulated):
        ...
        if percentage_gain < 8:
            if percentage_gain < 6:
                ...
                progress += percentage_gain
        ########################################################################
        scores.append(generate_score_for_simulation(sim, required_in_a_row))
    ...
    for i in subtract_scores_indices(board):
...
if __name__ == "__main__":
    ...
    if num_of_cols*num_of_rows >= 20 and len(game_board) > 2 and len(game_board[0]) > 2:
        while True:
    ...
    while True:
        apply_computer_turn(game_board, find_optimal_computer_turn(game_board, required_in_a_row_input, speed_turn_one))
